...
def download_snapshot(
    ...
    # BUG FIX 1: was f"Bearer{api_key}" — missing space caused 401 auth failures
    headers = {"Authorization": f"Bearer {api_key}"}
    ...
    try:
        ...
        data = response.json()
        print(f"Successfully downloaded {len(data) if isinstance(data, list) else 1} records")
        # BUG FIX 2: function never returned data — missing return statement caused None everywhere
        return data
    except Exception as e:
